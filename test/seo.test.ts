...
test("every tool says more about itself than four words, and not a paragraph", () => {
  // The ceiling is three lines on a card at three columns.
  for (const tool of [...SITE_TOOLS, ...EXTERNAL_TOOLS]) {
    ...
    assert.ok(tool.about.length <= 120, `${name}: ${tool.about.length} chars`);
